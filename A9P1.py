...
def run(metar_filename):
    ...
    def draw_name(name):
        x, y = 100, 100
        canvas.create_text(x, y, text=name, font=('Courier', 28), fill="red", anchor='nw')
        return
    ...
    def draw_date(time):
        ...
        canvas.create_text(x, y+52, text=text_time, font=('Courier', 28), fill="blue", anchor='nw')
        return
    # (rborden4) Wind gauge
    def draw_wind(speed, gust, angle):
        ...
        if(speed): # If nonzero
            ...
            canvas.create_text(x, y+h+12, text=text_speed, font=("Courier", 12), fill="black", anchor="w")
        else: # Otherwise print calm
            canvas.create_text(x, y+h+12, text="CALM", font=("Courier", 12), fill="black", anchor="w")
        # Display gust 
        if(gust):
            gust = round(gust*1.15)
            text_gust  = "Gust: " + str(gust) + "MPH"
            canvas.create_text(x, y+h+30, text=text_gust, font=("Courier", 12), fill="black", anchor="w")
        # Display angle
        if(angle):
    ...
    def draw_temp(temp, dewp):
        ...
        # (hortizme) Warning message
        if temp > 90: 
          ht = "Temperature exceeds 90F"
          # Display warning
          canvas.create_text(400, 50,   text=ht, font=("Courier", 18), fill="red")
        elif temp < 0: 
          lt = "Temperature is under 0F"
          # Display warning
          canvas.create_text(400, 50,   text=lt, font=("Courier", 18), fill="red")
        ...
        text_temp = "%.1fF" % temp
        text_dewp = "%.1fF" % dewp
        canvas.create_text(x+w/2, y+h+18,   text=text_temp, font=("Courier", 12), fill="red")
        canvas.create_text(x+w/2, y+h+2*22, text=text_dewp, font=("Courier", 12), fill="blue")
        return
    # (rali1) Altimeter
    def draw_alt(alt):
        # Sizing variables
    ...
    def draw_vis(vis):
        ...
        # (hortizme) Warning message
        if vis > 10: 
          hv = "Visibility exceeds 10SM"
          # Display warning
          canvas.create_text(400, 550,   text=hv, font=("Courier", 18), fill="red")
        elif vis < 0: 
          lv = "Visibility gauge malfunction"
          # Display warning
          canvas.create_text(400, 550,   text=lv, font=("Courier", 18), fill="red")
